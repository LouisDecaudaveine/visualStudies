class Grid{
  ...
  Grid(){
    for(int x = 0;x<widthRep;x++){
      for(int y = 0;y<heightRep;y++){
        world.add(new Box(boxSize,boxSize*y,boxSize*x));
      }
    }
  }
  ...
  void showTest(){
    for(int i = 0; i<world.size();i++){
      textSize(10);
      text(String.valueOf(i),world.get(i).x,world.get(i).y+boxSize);
    }
  }
  void updateAll(ArrayList<float[]> inBlob){
    for(int i = 0; i<world.size(); i++){
      int ind = contains(inBlob,i);
      if(ind >0){
        int intensi =  world.get(i).intensity;
        world.get(i).setIntensity(floor(intensi-map(inBlob.get(ind)[1],0,1,0,215)));
      }
  ...
  int contains(ArrayList<float[]> list, int p){
    for(int i = 0; i<list.size();i++){
      if((int)list.get(i)[0] == p)return i;
    }
    return -1;
  }
