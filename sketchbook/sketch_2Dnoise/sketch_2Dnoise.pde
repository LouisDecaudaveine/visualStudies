...
float[] zVals = new float[polygones.size()];
...
void setup(){
 ...
 for(int i = 0; i<zVals.length; i++){
   zVals[i] = red(polyH(polygones.get(i)));
 }
}
...
void draw(){
  ...
  ArrayList<int[]> rdmPoly = polygones.get(65);
  showPoly(rdmPoly);
  
  //ellipse(center[1],center[0],5,5);
  int polyH = polyH(rdmPoly);
  //println(polyH);
  //This displays all the polygones
  //println(polygones.size());
  //for(int i = 0; i < polygones.size();i++){
  //  showPoly(polygones.get(i));
  //}
...
void showPoly(ArrayList<int[]> polygone){
  beginShape();
  stroke(255);
...
int polyH(ArrayList<int[]> polygone){
  ...
  if(minX<5){
    ...
    for(int cX = width; cX>0; cX--){
      ...
      if(counter >0){
        return pixels[cX-2+width*y];
      }
      pixels[cX+width*y] = color(255);
      updatePixels();
    }
  }else{
